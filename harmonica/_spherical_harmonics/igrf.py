...
class IGRF14:
    ...
    def predict(self, coordinates):
        ...
        if n_data == 1:
            b_vector = tuple(float(c) for c in b_vector)
        return b_vector
    ...
    def grid(self, region, height, shape=None, spacing=None, adjust="spacing"):
        ...
        grid.attrs["title"] = f"IGRF14 magnetic field on {self.date.isoformat()}"
        grid.attrs["crs"] = self.ellipsoid.name
        grid.attrs["source"] = (
            "Generated by spherical harmonic synthesis using library "
            f"Harmonica (fatiando.org/harmonica) version {__version__}."
        )
        grid.attrs["description"] = (
            "Three components of the magnetic field vector "
            f"calculated on {self.date.isoformat()} "
            "in a geodetic coordinate system."
        )
        grid.attrs["units"] = "nT"
        grid.attrs["date"] = self.date.isoformat()
        ...
        grid.b_east.attrs["description"] = (
            "Eastward component of the magnetic field vector calculated on "
            f"{self.date.isoformat()}"
        )
        grid.b_north.attrs["description"] = (
            "Northward component of the magnetic field vector calculated on "
            f"{self.date.isoformat()}"
        )
        grid.b_up.attrs["description"] = (
            "Upward component of the magnetic field vector calculated on "
            f"{self.date.isoformat()}"
        )
        return grid
